...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler
{
    ...
    for (int i = 0; i < _delegates.count; i++) {
        id pointer = [_delegates pointerAtIndex:i];
        if ([pointer respondsToSelector:@selector(userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:)] && ![NSStringFromClass([pointer class]) isEqualToString:@"EXUserNotificationManager"]) {
...
- (void)addDelegate:(id<EXNotificationsDelegate>)delegate
{
    ...
    if ([delegate respondsToSelector:@selector(userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:)]) {
        ...
        if(![NSStringFromClass([delegate class]) isEqualToString:@"EXUserNotificationManager"]) {
...
{
    NSDictionary* tokenDict = [NSDictionary dictionaryWithObject:fcmToken forKey:@"token"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"EXDidRegisterForRemoteNotifications" object:nil userInfo:tokenDict];
}
